...
function portableRestoreRoundTripAndSecondRestore(logger) {
    ...
    for (var pass = 0; pass < 3; pass += 1) {
        ...
        store.recover();
        Test.assert(
            selected.get("pullUps") == 6 && selected.get("totalSets") == 3
        );
        Test.assert(
            selected.get("incrementAmount") == 3 &&
                selected.get("restDuration") == 90
        );
...
function interruptedRestoreNeverSelectsPartialData(logger) {
    ...
    for (var failure = 1; failure <= 24; failure += 1) {
        ...
        var store = new WorkoutStore(selected);
        store.recover();
        if (selected.prefix.equals("")) {
            Test.assert(selected.get("pullUps") == 16 && store.count() == 0);
        } else {
            Test.assert(
                selected.get("pullUps") == 6 && store.load(1).size() == 105
            );
        }
        // A retry can clean the partial inactive slot and finish normally.
        var retry = new WorkoutRestore(raw);
        for (var i = 0; i < records.size(); i += 1) {
            retry.accept(records[i]);
        }
        retry.commit();
        Test.assert(
            (new WorkoutStore(new SelectedWorkoutStorage(raw, null))).count() ==
                1
        );
    }
    return true;
