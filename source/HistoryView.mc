...
class HistoryView extends WatchUi.CustomMenu {
    ...
    function initialize(sets as Lang.Array) {
        ...
        CustomMenu.initialize(125, Graphics.COLOR_BLACK, {
            :title => new HistoryLabel("HISTORY"), :titleItemHeight => 65,
            :footerItemHeight => 85, :focus => focus
        });
        if (sets.size() == 0) {
            addItem(new WatchUi.CustomMenuItem(:empty, { :drawable => new HistoryLabel("NO SETS") }));
        }
        for (var i = 0; i < sets.size(); i += 1) {
            addItem(new WatchUi.CustomMenuItem(i, {
                :drawable => new HistorySetDrawable(sets[i] as Lang.Dictionary, i == sets.size() - 1)
            }));
        }
    }
}
...
class HistoryLabel extends WatchUi.Drawable {
    var text;
    function initialize(label) { Drawable.initialize({}); text = label; }
...
class HistoryDeleteLabel extends WatchUi.Drawable {
    ...
    function draw(dc) as Void {
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        dc.clear();
        dc.drawText(dc.getWidth() / 2, dc.getHeight() / 2 - 8,
            Graphics.FONT_XTINY, "Delete Workout",
            Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
    }
}
...
class HistorySetDrawable extends WatchUi.Drawable {
    ...
    function initialize(set as Lang.Dictionary, latest) {
        Drawable.initialize({}); entry = set; isLatest = latest;
    }
    function draw(dc) as Void {
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        dc.clear();
        var y = 8;
        var accentColor = isLatest ? Graphics.COLOR_YELLOW : Graphics.COLOR_BLUE;
        var detailColor = isLatest ? Graphics.COLOR_WHITE : Graphics.COLOR_LT_GRAY;
        var totalColor = isLatest ? Graphics.COLOR_YELLOW : Graphics.COLOR_WHITE;
        var separatorColor = isLatest ? Graphics.COLOR_YELLOW : Graphics.COLOR_DK_GRAY;
